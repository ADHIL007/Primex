...
const LoginLoading = () => {
  ...
  if (store.getState().CurrentSchoolData) {
    fetchRate();
  }
  return (
    <View style={styles.lottiecontainer}>
      <LottieView
        source={require('../assets/gifs/Loginsuccess.json')}
        autoPlay
        loop
        style={{flex: 1, width: 600, height: 600}}
      />
      <Text style={styles.lottieText}>Login SuccessFull</Text>
      <Text style={[styles.lottieText, {fontSize: 30, top: '70%'}]}>
        Redirecting...
      </Text>
    </View>
  );
